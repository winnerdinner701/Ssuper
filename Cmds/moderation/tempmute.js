...
module.exports = {
	...
	run: async (bot, message, args) => {
		...
		const memberRole = message.guild.roles.cache.find(r => r.name.includes('Member', "✦ | Community"));
		...
		message.channel.send(`You have muted ${mentionedMember} for ${time}.`);

		setTimeout(async () => {
			await mentionedMember.roles.add(memberRole.id);
			await mentionedMember.roles.remove(mutedRole.id);
			await mentionedMember.send('You have been unmuted');
		}, ms(time));
